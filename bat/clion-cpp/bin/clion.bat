...
:: Try (in order): CL_JDK, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
IF EXIST "%CL_JDK%" SET JDK=%CL_JDK%
IF NOT "%JDK%" == "" GOTO jdk
...
SET MAIN_CLASS_NAME=%CL_MAIN_CLASS_NAME%
IF "%MAIN_CLASS_NAME%" == "" SET MAIN_CLASS_NAME=com.intellij.idea.Main

IF NOT "%CL_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%CL_PROPERTIES%"

:: ---------------------------------------------------------------------
:: Collect JVM options and properties.
:: ---------------------------------------------------------------------
SET VM_OPTIONS_FILE=%CL_VM_OPTIONS%
IF "%VM_OPTIONS_FILE%" == "" SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\clion%BITS%.exe.vmoptions
SET ACC=
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_CL_%%p.log" "-Xbootclasspath/a:%IDE_HOME%/lib/boot.jar" -Didea.paths.selector=clion10 %IDE_PROPERTIES_PROPERTY%
SET IDE_JVM_ARGS=-Didea.platform.prefix=CLion -Didea.no.jre.check=true
...
IF NOT "%CL_CLASS_PATH%" == "" SET CLASS_PATH=%CLASS_PATH%;%CL_CLASS_PATH%
...
:error
ECHO ERROR: cannot start CLion.
ECHO No JDK found. Please validate either CL_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
ECHO
PAUSE
