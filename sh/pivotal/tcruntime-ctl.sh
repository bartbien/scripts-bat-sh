...
testJAVA_VERSION () {
  if [ $TC_MAJOR_VERSION -eq "8" -a $JAVA_VERSION -lt 17 ]; then
    echo "ERROR: Tomcat Version 8 Requires Java 7 or newer"
    exit 1
...
instance_install() {
    INITD_PATH="/etc/init.d/tc-server-${INSTANCE_NAME}"
    if [ "$(id -u)" != "0" ]; then
    ...
    ln -sf "${INSTANCE_BASE}/${INSTANCE_NAME}/bin/init.d.sh" "${INITD_PATH}"
    chkconfig "tc-server-${INSTANCE_NAME}" on
    echo "Service tc-server-${INSTANCE_NAME} has been created and will run as user tcserver"
}
...
if [ "$COMMAND" != "start" ] && [ "$COMMAND" != "run" ] && [ "$COMMAND" != "stop" ] && [ "$COMMAND" != "restart" ] && [ "$COMMAND" != "status" ] && [ "$COMMAND" != "verbose-status" ] && [ "$COMMAND" != "install" ]; then
    echo " "
